...
public class DAG12_Menu_Control : MonoBehaviour
{
    //=-----------------=
    // Public variables
    //=-----------------=
    ...
    private void Update()
    {
        if (menuEnabled)
        {
            if (previousMenu != null)
            {
                if (scrollStrings)
                {
                    previousMenu.GetComponent<DAG12_Menu_Control>().textTargetObjects[0].text = previousMenu.GetComponent<DAG12_Menu_Control>().baseText[0];
                }
            }
            if (nextMenu != null)
            {
                if (scrollStrings)
                {
                    nextMenu.GetComponent<DAG12_Menu_Control>().textTargetObjects[0].text = nextMenu.GetComponent<DAG12_Menu_Control>().baseText[0];
                }
            }
            ...
            if (!horizontalScrolling)
            {
                ...
                {
                    previousMenu.GetComponent<DAG12_Menu_Control>().enabled = true;
                    previousMenu.GetComponent<DAG12_Menu_Control>().onMenuChange.Invoke();
                    previousMenu.GetComponent<DAG12_Menu_Control>().currentSelection = gameObject.GetComponent<DAG12_Menu_Control>().currentSelection;
                    gameObject.GetComponent<DAG12_Menu_Control>().enabled = false;
                }

                // Right
                if (Input.GetKeyDown(inputManager.controls["Right"]) && nextMenu != null)
                {
                    nextMenu.GetComponent<DAG12_Menu_Control>().enabled = true;
                    nextMenu.GetComponent<DAG12_Menu_Control>().onMenuChange.Invoke();
                    nextMenu.GetComponent<DAG12_Menu_Control>().currentSelection = gameObject.GetComponent<DAG12_Menu_Control>().currentSelection;
                    gameObject.GetComponent<DAG12_Menu_Control>().enabled = false;
                }   
            }
            ...
            else if (!scrollStrings && scrollSprites)
            {
                ...
                {
                    Debug.LogError("DASDK: In [" + this.gameObject.name + "] under DAG12_Menu_Control, the spriteTargetObject did not have a SpriteRenderer or Image component attached! One of these is required for SpriteScrolling!");
                }
            }
            ...
            else if (scrollStrings && scrollSprites)
            {
                ...
                {
                    Debug.LogError("DASDK: In the game object [" + this.gameObject.name + "] under DAG12_Menu_Control, the spriteTargetObject did not have a SpriteRenderer or Image component attached! One of these is required for SpriteScrolling!");
                }
            }
    ...
    private void ErrorCheck()
    {
        ...
        {
            Debug.LogError("DASDK: In the game object [" + this.gameObject.name + "] under DAG12_Menu_Control, the length of Sprite Scrolling and Text Scrolling must be the same!");
        }
    }

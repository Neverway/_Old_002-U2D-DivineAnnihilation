...
public class NUPTopdownController : MonoBehaviour
{
    ...
    {
        // Horizontal control
        if (Input.GetKey(inputManager.controls["Right"]) && horizontalMovement < 1)                         { horizontalMovement += 1; }
        if (Input.GetKey(inputManager.controls["Left"]) && horizontalMovement > -1)                         { horizontalMovement -= 1; }
        if (!Input.GetKey(inputManager.controls["Left"]) && !Input.GetKey(inputManager.controls["Right"]))  { horizontalMovement = 0; }

        // Vertical control
        if (Input.GetKey(inputManager.controls["Up"]) && verticalMovement < 1)                              { verticalMovement += 1; }
        if (Input.GetKey(inputManager.controls["Down"]) && verticalMovement > -1)                           { verticalMovement -= 1; }
        if (!Input.GetKey(inputManager.controls["Down"]) && !Input.GetKey(inputManager.controls["Up"]))     { verticalMovement = 0; }
    ...
    private void PlayerMovement()
    {
        ...
        //     if (isGrounded)
        //     {
        //         playerRigidbody.AddForce(playerRigidbody.position * movementSpeed * movementMultiplier, ForceMode2D.Force); 
        //     }
        //     else if (!isGrounded)
        //     {
        //         playerRigidbody.AddForce(playerRigidbody.position * movementSpeed * movementMultiplier * airMultiplier, ForceMode2D.Force);
        //     }
        // }
        // else
        // {
    ...
    private void Sprint()
    {
        if (Input.GetKey(inputManager.controls["Action"]) && isGrounded)
        {
            sprinting = true;
            if (!oldPhysics)
            {
